...
class VGG16C1(nn.Module):
    def __init__(self, in_channels=3, channels=64, num_classes=10):
        ...
        self.block3 = nn.Sequential(
            ...
            nn.ReLU(inplace=True),
            nn.Conv2d(in_channels=channels * 4, out_channels=channels * 4, kernel_size=1, stride=1, padding=1),
            nn.ReLU(inplace=True),
            nn.MaxPool2d(kernel_size=2)
        )
        self.block4 = nn.Sequential(
            ...
            nn.ReLU(inplace=True),
            nn.Conv2d(in_channels=channels * 8, out_channels=channels * 8, kernel_size=1, stride=1, padding=1),
            nn.ReLU(inplace=True),
            nn.MaxPool2d(kernel_size=2)
        )
